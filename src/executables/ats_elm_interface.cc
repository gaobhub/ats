...
Implementation for the Ats_elm_interface.  Ats_elm_interface is basically just a class to hold
...
Ats_elm_interface::Ats_elm_interface(Teuchos::ParameterList& parameter_list,
  ...
  ats_elm_interface_init();

  vo_ = Teuchos::rcp(new Amanzi::VerboseObject("Ats_elm_interface", *parameter_list_));
};

void Ats_elm_interface::ats_elm_interface_init() {
  ats_elm_interface_list_ = Teuchos::sublist(parameter_list_, "cycle driver");
  read_parameter_list();

  // create the top level PK
  Teuchos::RCP<Teuchos::ParameterList> pks_list = Teuchos::sublist(parameter_list_, "PKs");
  Teuchos::ParameterList pk_tree_list = ats_elm_interface_list_->sublist("PK tree");
...
void Ats_elm_interface::setup() {
  // Set up the states, creating all data structures.
  S_->set_time(t0_);
  S_->set_cycle(cycle0_);
  S_->RequireScalar("dt", "ats_elm_interface");
...
double Ats_elm_interface::initialize() {
  ...
  *S_->GetScalarData("dt", "ats_elm_interface") = 0.;
  S_->GetField("dt","ats_elm_interface")->set_initialized();
  S_->InitializeFields();
  ...
  if (ats_elm_interface_list_->isSublist("required times")) {
    Teuchos::ParameterList& sublist = ats_elm_interface_list_->sublist("required times");
...
void Ats_elm_interface::finalize() {
...
void Ats_elm_interface::report_memory() {
...
void Ats_elm_interface::read_parameter_list() {
  Amanzi::Utils::Units units;
  t0_ = ats_elm_interface_list_->get<double>("start time");
  std::string t0_units = ats_elm_interface_list_->get<std::string>("start time units", "s");
  if (!units.IsValidTime(t0_units)) {
    Errors::Message msg;
    msg << "Ats_elm_interface start time: unknown time units type: \"" << t0_units << "\"  Valid are: " << units.ValidTimeStrings();
    Exceptions::amanzi_throw(msg);
  }
  bool success;
  t0_ = units.ConvertTime(t0_, t0_units, "s", success);

  t1_ = ats_elm_interface_list_->get<double>("end time");
  std::string t1_units = ats_elm_interface_list_->get<std::string>("end time units", "s");
  if (!units.IsValidTime(t1_units)) {
    Errors::Message msg;
    msg << "Ats_elm_interface end time: unknown time units type: \"" << t1_units << "\"  Valid are: " << units.ValidTimeStrings();
    Exceptions::amanzi_throw(msg);
  }
  t1_ = units.ConvertTime(t1_, t1_units, "s", success);

  max_dt_ = ats_elm_interface_list_->get<double>("max time step size [s]", 1.0e99);
  min_dt_ = ats_elm_interface_list_->get<double>("min time step size [s]", 1.0e-12);
  cycle0_ = ats_elm_interface_list_->get<int>("start cycle",0);
  cycle1_ = ats_elm_interface_list_->get<int>("end cycle",-1);
  duration_ = ats_elm_interface_list_->get<double>("wallclock duration [hrs]", -1.0);
  subcycled_ts_ = ats_elm_interface_list_->get<bool>("subcycled timestep", false);

  // restart control
  restart_ = ats_elm_interface_list_->isParameter("restart from checkpoint file");
  if (restart_) restart_filename_ = ats_elm_interface_list_->get<std::string>("restart from checkpoint file");
}
...
// -----------------------------------------------------------------------------
double Ats_elm_interface::get_dt(bool after_fail) {
  ...
  if (dt < min_dt_) {
    Errors::Message message("Ats_elm_interface: error, timestep too small");
    Exceptions::amanzi_throw(message);
  }
...
bool Ats_elm_interface::advance(double t_old, double t_new, double& dt_next) {
...
void Ats_elm_interface::visualize(bool force) {
...
void Ats_elm_interface::checkpoint(double dt, bool force) {
...
// timestep loop
// -----------------------------------------------------------------------------
void Ats_elm_interface::cycle_driver() {
  ...
  try {
    ...
    while ((S_->time() < t1_) &&
      ...
      *S_->GetScalarData("dt", "ats_elm_interface") = dt;
      *S_inter_->GetScalarData("dt", "ats_elm_interface") = dt;
      *S_next_->GetScalarData("dt", "ats_elm_interface") = dt;
